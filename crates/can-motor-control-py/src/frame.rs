//! Python wrapper for `can_motor_control::CanFrame`.

use can_motor_control::{CanFrame, FrameFlags};
use pyo3::prelude::*;
...
#[pyclass(name = "CanFrame", module = "can_motor_control")]
#[derive(Clone)]
...
impl PyCanFrame {
    ...
    fn classical(id: u32, payload: &[u8]) -> PyResult<Self> {
        let inner = CanFrame::classical(id, payload)
            .map_err(|e| transport_to_pyerr(can_motor_control::TransportError::FrameError(e)))?;
        Ok(Self { inner })
    }
