...
- [x] 1.1 Delete `FdNotImplementedInV1` from `crates/dm-control/src/transport/mod.rs` (`TransportError`) and from `crates/dm-control/src/error.rs` (`Error`), plus their `#[error("...")]` strings.
...
- [x] 1.3 Remove the config-parse rejection loop (`if bus.fd { return Err(FdNotImplementedInV1 ...) }`) in `crates/dm-control/src/config.rs`.
...
- [x] 3.2 In the decode path (`dm-codec/src/codec.rs`), stop discarding FD frames unconditionally: accept FD-format state frames when bound to an FD bus, keep the `len != 8`/`is_fd` discard for classical binding so classical behavior is unchanged.
...
- [x] 5.2 `crates/dm-control-py/src/transport.rs`: update `SocketCanBus(..., fd=True)` docstring (no longer raises); `crates/dm-control-py/src/lib.rs`: update `ConfigError` doc wording that says "unsupported option such as CAN-FD in v1".
...
- [x] 6.1 Flip `crates/dm-control/tests/config_loader.rs::fd_true_rejected_at_parse_time` to assert the robot builds and the bus is FD-capable (rename the test accordingly).
...
- [x] 9.1 Expose `MockCanBus.new_fd(name)` in PyO3 (`crates/dm-control-py/src/transport.rs`) and the `.pyi` stub; Rust `MockCanBus::new_fd` already exists.
...
- [x] 9.7 Verify: `cargo build -p dm-control --examples` + clippy clean; `pytest tests/python` green (14 passed); example `--help`/`--fd` and `dev_api_smoke --mock --fd` smoke-checked.
