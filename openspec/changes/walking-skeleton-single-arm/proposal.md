...
- Establish a Cargo workspace with four crates: `motor-codec` (pure, no IO — the vendor-agnostic codec trait + common types), `dm-codec` (Damiao implementation of the trait), `dm-control` (transport + motor + group + robot, generic over codec via trait objects), `dm-control-py` (PyO3 binding built by maturin).
...
- Set up CI for: cargo fmt/clippy/test on `dm-can-codec` and `dm-control` (mock transport, no real hardware), maturin wheel build for `dm-control-py`, Python smoke test against the mock bus.
...
- `python-bindings`: PyO3 wrapper crate `dm-control-py`, maturin build, the Python-facing class hierarchy (`Robot`, `RobotBuilder`, `SocketCanBus`, `MotorSpec`, kind-dispatched `PyArm` / `PyGripper`), numpy roundtrip for batch state and commands, GIL released during blocking calls, context-manager protocol on `Robot`. Bindings ship Damiao support in this change; future vendor codecs surface as additional submodules (`dm_control.vendors.robostride`, etc.).
...
- **Code**: Net-new repository. New `Cargo.toml` workspace at root; new crates `crates/motor-codec`, `crates/dm-codec`, `crates/dm-control`, `crates/dm-control-py`; new `python/dm_control/` package skeleton; new `configs/` directory with one example TOML; new `examples/` for both Rust and Python.
- **Public APIs**: All net-new. Two parallel surfaces: Rust API rooted at `dm_control::{Robot, RobotBuilder, Arm, Gripper, CanBus, SocketCanBus, ...}` and Python API rooted at `dm_control.{Robot, RobotBuilder, SocketCanBus, MotorSpec, ...}` mirroring the Rust shape with PEP 8 names.
