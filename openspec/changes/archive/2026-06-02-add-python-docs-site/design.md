...
`dm_control` is a PyO3 extension: the user-visible Python classes (`Robot`, `Arm`, `Gripper`, …) are implemented in Rust under `crates/dm-control-py/src/` and re-exported by a thin pure-Python package at `crates/dm-control-py/python/dm_control/`. The typed surface lives in `.pyi` stubs (`__init__.pyi`, `damiao.pyi`); they carry signatures but no prose. The Rust `#[pymethods]` carry only internal notes (e.g. GIL rationale), so there are no user-facing docstrings today and `help()` returns nothing useful.
...
Because the classes are a compiled extension, griffe must **import** `dm_control` (after `maturin develop`) to read the runtime `__doc__` that PyO3 generates from the Rust `///` comments. This guarantees the rendered prose is exactly what `help()` shows — one source of truth, no drift.
...
### Decision: consistent `__module__ = "dm_control"` across all exported types

griffe resolves the package's re-exports (`from dm_control._native import …`) by following each object's `__module__`. The `#[pyclass]` types already set `module = "dm_control"`, but the exceptions were created with `create_exception!(_native, …)`, stamping `__module__ = "_native"` — a bare name griffe cannot locate, which broke alias resolution for every error type. The fix is to create them with `create_exception!(dm_control, …)` so all exported types agree on `dm_control`. This is also more correct for Python: `repr(DmError)` now reads `<class 'dm_control.DmError'>`, matching where users import it.

### Decision: one local command, uv-managed reproducible environment

`make docs` / `make docs-build` are the single entry points. They run `uv sync --reinstall-package dm_control` (build the extension editable via maturin + install the locked toolchain) then `uv run mkdocs serve` / `mkdocs build`. The environment is a **uv project** rooted at the repo root: a non-package `pyproject.toml` (`[tool.uv] package = false`) that depends on `dm_control[docs]` via an editable path source, pinned by a committed `uv.lock`. This makes local and (future) CI builds byte-for-byte reproducible — CI is `uv sync --frozen` + `uv run mkdocs build`, no extra glue.
...
- **griffe alias resolution across the static `__init__.py` / compiled `_native` boundary** (the sharp edge anticipated here) → resolved by `force_inspection: true` plus consistent `__module__ = "dm_control"` on every exported type. Verified end-to-end: all 17 exported classes resolve with docstrings and render with signatures.
...
- **Rust-docstring edits need an explicit rebuild** (uv won't auto-detect Rust source changes for an editable install) → `make docs*` uses `uv sync --reinstall-package dm_control`, forcing the rebuild every run (a cargo no-op when nothing changed).
