...
### New Capabilities
- `python-docs-site`: A locally-buildable documentation site for the `can_motor_control` Python package — API reference auto-generated from PyO3 docstrings plus the existing prose guides, driven by one command.
...
- **Source**: `crates/can-motor-control-py/src/*.rs` — add `///` docstrings (and `#[pyo3(text_signature)]` where griffe needs signatures not covered by `.pyi`).
- **New files**: `mkdocs.yml`, a `docs/` site structure (or `crates/can-motor-control-py/docs/`), API reference stub pages, a local build command/target.
