...
- [x] 1.1 Add a `docs` optional-dependency group (`mkdocs-material`, `mkdocstrings[python]`) in `crates/can-motor-control-py/pyproject.toml` (or a `requirements-docs.txt`), kept out of runtime deps
...
- [x] 2.1 Add user-facing `///` docstrings to `#[pyclass]` items in `crates/can-motor-control-py/src/`: `Robot`, `RobotBuilder`, `Arm`, `Gripper`, `Motor`, `MotorGroup`, `MotorSpec`, `CanFrame`, `MockCanBus`, `SocketCanBus`
...
- [x] 2.5 `maturin develop` and confirm `help(can_motor_control.Robot.tick)` (and a Jupyter `?` spot-check) shows the authored prose
...
- [x] 3.2 Configure the mkdocstrings python handler in import mode targeting the `can_motor_control` package, merging `.pyi` stubs for signatures
- [x] 3.3 Create the site pages: home/overview (from README), a Python API reference page using `::: can_motor_control`, and a rustdoc bridge page
...
- [x] 5.2 Verify every class in `can_motor_control.__all__` appears in the rendered API reference with rendered signatures
