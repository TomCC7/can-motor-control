...
- **Code**:
  - `crates/can-motor-control/src/transport/socketcan.rs` (open, send, caps; remove
    FD reject)
  - `crates/can-motor-control/src/transport/mod.rs` and `crates/can-motor-control/src/error.rs`
    (remove `FdNotImplementedInV1`)
  - `crates/can-motor-control/src/transport/mock.rs` (FD-capable mock)
  - `crates/can-motor-control/src/config.rs` (stop rejecting `fd = true`)
  - `crates/damiao-codec/src/codec.rs` (gate FD emit/decode on bound caps)
  - `crates/can-motor-control-py/src/transport.rs` + `src/lib.rs` (docstrings, error
    wording)
- **Configs/examples**: `configs/` gains an FD example; existing
  `fd = false` configs are unaffected.
- **Tests**: `crates/can-motor-control/tests/config_loader.rs` flips the
