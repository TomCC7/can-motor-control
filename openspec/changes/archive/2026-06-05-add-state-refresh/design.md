...
- `tick()` is receive-only (`crates/can-motor-control/src/robot.rs:155`; the
...
- A Damiao motor only replies with state **in response to a frame we send**
  (`damiao-codec` decode). With nothing sent after enable, no further replies arrive.
...
(`crates/damiao-codec/src/ext.rs:88` `encode_refresh`) but it is mislabeled "refresh
...
having `can-motor-control` downcast `Box<dyn MotorCodec>` to `DamiaoCodecExt`. *Why:*
