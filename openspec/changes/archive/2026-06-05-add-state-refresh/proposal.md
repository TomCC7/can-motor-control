...
- **`DamiaoCodec`**: implement `encode_refresh` returning the `0x7FF`/`0xCC`
  ...
  motion."
- **`can-motor-control`**: add `MotorGroup::refresh_all()` and `Arm`/`Gripper`/`Robot::refresh()`
...
- **Code**:
  ...
  - `crates/damiao-codec/src/codec.rs` (impl), `crates/damiao-codec/src/ext.rs` (doc fix)
  - `crates/can-motor-control/src/group.rs`, `src/robot.rs` (`refresh_all`/`refresh`)
  - `crates/can-motor-control-py/src/robot.rs` + `python/can_motor_control/__init__.pyi` (bindings)
