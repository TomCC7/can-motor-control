...
- [x] 1.2 Implement `encode_refresh` on `DamiaoCodec` (`crates/dm-codec/src/codec.rs`) returning `Ok(Some(..))` with the `0x7FF`/`0xCC` frame; reuse/delegate to the existing `DamiaoCodecExt::encode_refresh` bytes.
...
- [x] 3.1 Expose `Arm.refresh()`, `Gripper.refresh()`, `Robot.refresh()` in `crates/dm-control-py/src/robot.rs`, GIL-released (`Python::allow_threads`) like `tick`.
- [x] 3.2 Add the three `refresh()` signatures to `crates/dm-control-py/python/dm_control/__init__.pyi`.
...
- [x] 5.2 Rebuild the extension (`uv sync --reinstall-package dm_control`); `uv run --with pytest python -m pytest tests/python` green.
